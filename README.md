...
6. Изменил тип аутентификации учетной записи sys_temp с sha2 на парольную запросом и
переподключился к серверу от имени sys_temp.
```
ALTER USER 'sys_test'@'localhost' IDENTIFIED WITH mysql_native_password BY '12345';
```
7. По ссылке скачал дамп базы данных.
8. Создал пустую БД и восстановил в нее дамп (последовательно скрипты sakila-schema.sql и sakila-data.sql).
